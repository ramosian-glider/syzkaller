...
func (ctx *validCtx) validateArg(arg Arg, typ Type, dir Dir) error {
	...
	if ctx.currentCall != nil && ctx.currentCall.Meta.Attrs.Noany && ctx.target.isAnyPtr(arg.Type()) {
		return fmt.Errorf("ANY argument for noany call %v", ctx.currentCall.Meta.Name)
	}
	ctx.args[arg] = true
