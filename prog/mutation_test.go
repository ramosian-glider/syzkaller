...
func TestMutateNoany(t *testing.T) {
	target := initTargetTest(t, "test", "64")
	p, err := target.Deserialize([]byte(`mutate_noany(&(0x7f0000000000)={0x1, 0x2, 0x3, 0x4, "5e9ce23b"})`), Strict)
	if err != nil {
		t.Fatal(err)
	}
	rs := rand.NewSource(0)
	...
	for i := 0; i < 100; i++ {
		...
		ctx.p = p1
		if ctx.squashAny() {
			t.Fatalf("squashAny mutated a noany call: %s", p1.Serialize())
		}
	}
}
